...
impl WaylandState {
    ...
    /// Compile the keymap the compositor handed us and build an xkb_state for
    /// per-keycode character resolution. Without this, character keys stay
    /// unmapped (returning None from keycode_to_key).
    fn load_keymap(&mut self, fd: std::os::fd::OwnedFd) {
        let mut file = std::fs::File::from(fd);
        if let Some(km) = xkb::Keymap::new_from_file(
            &self.xkb_context,
            &mut file,
...
///
/// Special (non-character) evdev keycodes are layout-invariant — they map to
/// physical positions, so we look them up directly. Character keys are
/// resolved through xkb_state so they respect the user's keyboard layout
/// (AZERTY, Colemak, Dvorak, …) and active modifiers.
pub(super) fn keycode_to_key(kc: u32, xkb_state: Option<&xkb::State>) -> Option<Key> {
    ...
    // Character keys: ask xkb what character this keycode produces under the
    // current layout + modifier state. evdev keycodes are offset by 8 in the
    // XKB numbering.
    let state = xkb_state?;
